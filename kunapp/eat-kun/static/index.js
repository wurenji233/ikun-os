...
var audio=new Audio("./static/music/music.mp3");
audio.loop=true;
audio.preload="auto";
audio.volume=0.1;
...
(function(w) {
    ...
    function gameOver() {
        ...
        updatePanel();
        //依托答辩，但是没办法
        //奶奶滴为什么JavaScript没有sleep！JavaScript是世界上最差的编程语言！
        audio.volume=0.075;
        setTimeout(function(){
            audio.volume=0.05;
            setTimeout(function(){
    ...
    function shareText(cps) {
        if (mode === MODE_NORMAL) {
            ...
            if (!legalDeviationTime()) {
                return '错误：多' + ((deviationTime / 1000) - 20).toFixed(2) + "秒倒计时";
            }
            SubmitResults();
        }

        if (cps <= 5) return '你个纯鹿人！';
        if (cps <= 8) return '初级真ikun证明';
        if (cps <= 10)  return '高级真ikun证明';
        if (cps <= 15) return '高级真ikun证明';
        return '你们食不食人啊！';
    }
